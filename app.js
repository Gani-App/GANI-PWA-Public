...
async function loadDevelopmentStatus(){const gate=$('#devGate'),content=$('#devControlContent');if(!gate||!content)return;if(!devEnabled){gate.hidden=false;content.hidden=true;return}gate.hidden=true;content.hidden=false;try{const r=await fetch('./gani-development-status.json',{cache:'no-store'});if(!r.ok)throw new Error('Status ledger unavailable');const data=await r.json();renderDevelopmentStatus(data)}catch(e){$('#devOverallStatus').textContent='UNAVAILABLE';$('#devOverallDetail').textContent=e.message;$('#devJobGrid').innerHTML='<div class="mission-panel"><b>Status ledger unavailable</b><p class="mission-muted small">The control center could not load its non-sensitive status source.</p></div>'}}
...
function bind(){
  ...
  $('#frontFaceSignIn')?.addEventListener('click',()=>toast('Sign in is not configured yet'));
